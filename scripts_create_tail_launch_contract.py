#!/usr/bin/env python3
"""Create the immutable, policy-bound M9 GLM 5090-tail launch contract."""
...
def _write_new(path: Path, payload: bytes) -> None:
    _require(not path.exists(), f"refusing to overwrite launch contract {path}")
...
def main() -> None:
    """Validate immutable inputs and atomically create the shared tail contract."""
    ...
    _require(
        ...
        "CAL-50 gate is not bound to this environment class",
    )
    identity = gate.get("identity")
    _require(isinstance(identity, str), "CAL-50 gate has no generator identity")
    ...
    contract = {
        "schema": TAIL_CONTRACT_SCHEMA,
        "profile": "m9-glm-tail-v1",
        "authorized_shards": M9_TAIL_RANGES,
        "generator": "glm",
    ...
    print(f"tail launch contract: {args.output}")
